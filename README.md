...
Codesmith включает лёгкий HTTP-router для сценария, когда ComfyUI запущен на нескольких компьютерах с GPU, а работать хочется через один адрес. Важно: router **не делит один workflow по двум видеокартам**. Он распределяет новые `/prompt`-задания между отдельными ComfyUI-инстансами, поэтому лучше всего подходит для очереди генераций и параллельной работы нескольких клиентов.
...
После этого клиентам можно давать один адрес: `http://<router-pc-ip>:8190`. Статус backend-ов доступен по `http://<router-pc-ip>:8190/cluster/backends`.

### Практические замечания

- На обоих ComfyUI-ПК должны быть одинаковые custom nodes, модели и пути к моделям, иначе часть workflow будет падать только на одном backend-е.
